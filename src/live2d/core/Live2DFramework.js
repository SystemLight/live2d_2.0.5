...
L2DBaseModel.prototype.hitTestSimple = function (drawID, testX, testY) {
    ...
    let points = this.live2DModel.getTransformedPoints(drawIndex);
    ...
    let tx = this.modelMatrix.invertTransformX(testX);
    let ty = this.modelMatrix.invertTransformY(testY);
...
L2DEyeBlink.prototype.calcNextBlink = function () {
    let time /*long*/ = UtSystem.getUserTimeMSec();
...
L2DMotionManager.prototype.updateParam = function (model/*ALive2DModel*/) {
    let updated = MotionQueueManager.prototype.updateParam.call(this, model);
...
L2DPhysics.load = function (buf /*byte[]*/) {
    ...
    for (let i = 0; i < paramNum; i++) {
        ...
        let physics = new PhysicsHair(); //PhysicsHairPhysicsHair
...
L2DPose.prototype.updateParam = function (model/*ALive2DModel*/) {
    ...
    let curTime = UtSystem.getTimeMSec();
...
L2DPose.prototype.normalizePartsOpacityGroup = function (model/*ALive2DModel*/, partsGroup/*L2DPartsParam[]*/, deltaTimeSec/*float*/) {
    ...
    let CLEAR_TIME_SEC = 0.5;
    let phi = 0.5;
    let maxBackOpacity = 0.15;
...
L2DTargetPoint.prototype.update = function () {
    ...
    let FACE_PARAM_MAX_V = 40.0 / 7.5;
    let MAX_V = FACE_PARAM_MAX_V / L2DTargetPoint.FRAME_RATE;
    if (this.lastTimeSec === 0) {
        this.lastTimeSec = UtSystem.getUserTimeMSec();
        return;
    }
    let curTimeSec = UtSystem.getUserTimeMSec();
    let deltaTimeWeight = (curTimeSec - this.lastTimeSec) * L2DTargetPoint.FRAME_RATE / 1000.0;
    this.lastTimeSec = curTimeSec;
    let FRAME_TO_MAX_SPEED = TIME_TO_MAX_SPEED * L2DTargetPoint.FRAME_RATE;
    let MAX_A = deltaTimeWeight * MAX_V / FRAME_TO_MAX_SPEED;
